...
Створити тендер
    ...
    Click Element  xpath=//button[contains(@id, "create_auction_modal_btn")]
    Wait Element Animation  xpath=//select[contains(@id, "tenders-tender_method")]
    Select From List By Value  xpath=//select[contains(@id, "tenders-tender_method")]  open_${data.procurementMethodType}
    Click Element  xpath=//button[contains(@id, "disqualification")]
    ...
    \  rwsbank.Додати Предмет   ${item}  ${items[${item}]}
...
Додати предмет закупівлі
    ...
    rwsbank.Пошук Тендера По Ідентифікатору  ${tender_owner}  ${tender_uaid}
    ...
    rwsbank.Додати Предмет  ${item_number}  ${item_data}
...
Завантажити документ
    ...
    rwsbank.Завантажити документ в тендер з типом  ${tender_owner}  ${tender_uaid}  ${file_path}  clarifications


Завантажити ілюстрацію
    [Arguments]  ${tender_owner}  ${tender_uaid}  ${file_path}
    rwsbank.Завантажити документ в тендер з типом  ${tender_owner}  ${tender_uaid}  ${file_path}  illustration
...
Отримати інформацію із предмету
    ...
    ...  ELSE IF  '${field}' == 'unit.code'  Log To Console  ${red}\n\t\t\t Це поле не виводиться на rwsbank.byustudio.in.ua
