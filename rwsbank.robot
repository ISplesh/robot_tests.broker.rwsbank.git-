...
Додати актив до об'єкта МП
    ...
    rwsbank.Пошук об’єкта МП по ідентифікатору  ${username}  ${tender_uaid}
    ...
    Run Keyword And Ignore Error  rwsbank.Додати предмет МП  ${item_data}
    Run Keyword And Ignore Error  rwsbank.Scroll To And Click Element   id=btn-submit-form
    Wait Until Element Is Visible  xpath=//div[@data-test-id="tenderID"]
...
Внести зміни в об'єкт МП
    ...
    rwsbank.Пошук об’єкта МП по ідентифікатору  ${tender_owner}  ${tender_uaid}
    Click Element  xpath=//a[contains(@href, "asset/update")]
    Wait Until Element Is Visible  xpath=//form[@id="asset-form"]
...
Завантажити ілюстрацію в об'єкт МП
    ...
    rwsbank.Завантажити документ в об'єкт МП з типом  ${username}  ${tender_uaid}  ${filepath}  illustration
...
Завантажити документ в ставку
    ...
    rwsbank.Скасувати цінову пропозицію  ${username}  ${tender_uaid}
...
Завантажити протокол скасування в контракт
    [Arguments]  ${username}  ${tender_uaid}  ${file_path}  ${award_index}
    rwsbank.Завантажити протокол дискваліфікації в авард  ${username}  ${tender_uaid}  ${file_path}  ${award_index}
...
#     Wait Until Keyword Succeeds  30 x  10 s  Run Keywords
#     ...  Reload Page
#     ...  Element Should Be Visible   xpath=//div[contains(@data-test-id,"status") and not (contains(@data-test-id,"auction"))]
#     ...  Element Should Contain  xpath=//div[contains(@data-test-id,"status") and not (contains(@data-test-id,"auction"))]  Опубліковано
#     ${rectificationPeriod} = Get Text  xpath=//div[contains(@data-test-id="rectificationPeriod")]